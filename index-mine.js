...
class XMLHttpRequestMock {
  ...
  send(any) {
    ...
    console.log(self.onreadystatechange);
    console.log('- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -');
    ...
    let pdfData;
    try {
      self.response = fs.readFileSync(url);
      self.status = 200;
      self.readyState = XMLHttpRequestMock.DONE;
    } catch (error) {
      self.status = 400;
    }
    self.onreadystatechange();
...
XMLHttpRequestMock.DONE = Date.now();

global.XMLHttpRequest = XMLHttpRequestMock;

PDFJS.cMapPacked = true;
PDFJS.cMapUrl = `${__dirname}/node_modules/pdfjs-dist/cmaps/`;
